...
#### リバースギアとフラッシャー(ハザードランプ)を連動する
* リバースギアに入れた時に自動的にフラッシャー(ハザードランプ)を点灯させ、リバース解除時に消灯します。
  * リバースギアに入れる前にフラッシャー(ハザードランプ)が点灯している場合はこの設定を無視します。
    * リバース解除時のみフラッシャー(ハザードランプ)を消灯しないオプションもあります。
...
* 走行中の道路の制限速度に、クルーズコントロールの設定速度を自動調節します
  * CC 設定後、CC速度の上下を一度でも行うと、そのCC速度をキープします（制限速度に追従しなくなります）
  * CC を一度オフにするとリセットされ、再度 CC をオンにすると制限速度に合わせて追従します。
