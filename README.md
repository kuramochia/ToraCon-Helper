...
#### アクセルを踏んでいる状態でリターダーを戻す入力を行うと、リターダーを一気に全段戻す
* アクセルを踏んでいる状態(10%以上)でリターダーを戻す入力を行うと、リターダーを一気に全段戻します。
  * 加速してクルコン入れようとしたらリターダーがオンのままで怒られたとき、料金ゲート通過後の加速時にリターダーをオフにしたいときに便利です。
...
### その他 アクション設定
#### 制限速度に合わせてクルーズコントロールを自動設定
* 走行中の道路の制限速度に、クルーズコントロールの設定速度を自動調節します
* ATS で MPH 表記の場合は、`ATS で MPH を使う` をオンにしてください
* クルーズコントロールの増減をした時のステップをゲーム内で 1,3,5 から選べますが、アプリから取得できないため、同じ値を選択してください。
