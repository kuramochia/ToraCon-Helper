...
namespace SCSSdkClient.Demo {
    ...
    public partial class SCSSdkClientDemo : Form {
        ...
        private bool inputTestStarted = false;

        private Timer timer = new Timer() { Interval = 2000 };
        private void button_input_pause_Click(object sender, EventArgs e)
        {
            if (!inputTestStarted)
            {
                // start
                ...
                timer.Start();
                inputTestlabel.Text = "Timer Startted";
            }
            else
            {
                //stop
                timer.Stop();
                timer.Tick -= Timer_Tick;
                inputTestlabel.Text = "Timer Stoped";
            }
